  ...
  <div>
    <div v-if="questions">
      ...
      <div class="question-bg">
        <QuestionComp :question="questions[currentIndex-1]" @pickAnswer="pickAnswer" />
      </div>
  ...
  methods: {
    ...
    handleClick() {
      ...
      if (this.currentIndex <= this.questions.length) {
        //存储答案
        this.storeAnswer(this.checkedAnswer);
        this.increament();
    ...
    pickAnswer(value) {
      console.log("checked answer=", value);
      this.checkedAnswer = value;
    }
  },
  computed: mapGetters(["questions", "currentIndex"]),
  created() {
    this.initialState();
    //导航完成后获取数据
    this.fetchQuestions();
